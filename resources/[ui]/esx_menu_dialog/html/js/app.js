	...
	let MenuTpl =
		'<div id="menu_{{_namespace}}_{{_name}}" class="dialog {{#isBig}}big{{/isBig}}">' +
			'<div class="head"><span>{{title}}</span></div>' +
				'{{#isDefault}}<input type="text" name="value" placeholder="1" autofocus>{{/isDefault}}' +
				'<button type="button" name="submit" autofocus>Envoyer</button>' +
				'<button type="button" name="cancel" autofocus>Annuler</button>' +
			'</div>' +
		'</div>'
	;
