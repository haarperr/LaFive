...
Citizen.CreateThread(function()
	...
	LRKM.Open = function(namespace, name, data, submit, cancel, change, close)
		...
		LRKM.Opened[namespace] = LRKM.Opened[namespace] or {}

		LRKM.Opened[namespace][name] = data
	...
	LRKM.Close = function(namespace, name)

		if LRKM.Opened[namespace] then

			if LRKM.Opened[namespace][name] then

				LRKM.Opened[namespace][name].handle:GoBack()

				LRKM.Opened[namespace][name] = nil
	...
	LRKM.Update = function(namespace, name, query, newData)

		local menu = LRKM.Opened[namespace][name]
	...
	ESX.UI.Menu.RegisterType(MenuType, LRKM.Open, LRKM.Close, LRKM.CloseAll, LRKM.Update)
