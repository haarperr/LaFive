    ...
    while true do
        ...
        if IsControlJustPressed(1, 38) and callActive then
			...
			if isCall == false then
				...
				TriggerEvent('esx:showAdvancedNotification', source, '~p~Appel !', '~g~Position enregistré, ~b~Vous avez pris l\'appel !', 'CHAR_CALL911', 23)
...
--	SendNotification("Appuyez sur ~b~Flèche de gauche~s~ pour prendre l'appel ou ~r~Flèche de droite~s~ pour le refuser")
    TriggerEvent('esx:showAdvancedNotification', source, '~p~Appel !', '~g~Vous avez reçu un apelle !', 'CHAR_CALL911', 23)
