...
local VoiceMode = {
	...
	{ dist = 8, message = "" },
	{ dist = 14, message = "" },
	{ veh = true, dist = 4, func = function(ped) return IsPedInAnyVehicle(ped) end, message = "" },
}
...
RegisterNetEvent('r_radio:mute')
AddEventHandler('r_radio:mute', function(bool,sour)

	Voice.ListenersRadio[sour] = bool
end)
RegisterNetEvent('r_radio:SyncRadio')
AddEventHandler('r_radio:SyncRadio', function(azz,bool)
	for i = 1,#azz,1 do
	...
	while true do
        ...
        if IsControlPressed(0, Keys["HOME"]) then
                ESX.TriggerServerCallback('esx_cryptedphone:getItemAmount', function(qtty)
					if not ESX.UI.Menu.IsOpen('radio', GetCurrentResourceName(), 'main') and qtty > 0 then
						ESX.UI.Menu.CloseAll()
						OpenRadioMenu()
					else
						drawnotifcolor("Tu n'as pas de Talkie Walkie.", 26)
					end
				end, 'radio')
            
        end
		if IsControlPressed(1, 1000) then
...
RegisterNetEvent('r_radio:SyncRadio2')
AddEventHandler('r_radio:SyncRadio2', function(azz)
...
function OpenRadioMenu()
    ...
    if radioMenu == nil then
       	   radioMenu = NativeUI.CreateMenu("Talkie Walkie", "", 5, 200,nil,nil,nil,0,0,0,200)
...
function AddRadioMenu(menu)
    ...
    pp = NativeUI.CreateItem('Fréquence', "Ici tu met la fréquence de ta radio", 5, 200,nil,nil,nil,0,0,0,nil)
    pp:RightLabel(freq)
    menu:AddItem(pp)
    menu.OnItemSelect = function(m, i, ind)
        if ind == 1 then
            drawnotifcolor("Met un fréquence entre 0 et 100", 26)
            dm = gettxt2(freq)
            drawnotifcolor("La fréquence est régle sur : " .. tostring(dm), 18)
            dm = tonumber(dm)
            if dm ~= nil then
                if dm > 0 then
                    dm = round2(dm, 2)
                    if dm <= 100 then
                        ...
                        drawnotifcolor("Met un fréquence entre 0 et 100", 6)
                    end
                end
            end
        end
        ...
        if ind == 2 then
            --drawnotifcolor("Le fréquence est definis sur : " .. freq, 18)
            TriggerServerEvent("r_radio:SetFreq", freq)
			_menuPool:RefreshIndex()
			currentFreq = freq
			if ppks == nil then
			--menu:RemoveItemAt(3)
    ...
    menu.OnCheckboxChange = function(_, _, checked)
        TriggerServerEvent("r_radio:ToggleRadio", freq, checked)
    end
    _menuPool:RefreshIndex()
end
