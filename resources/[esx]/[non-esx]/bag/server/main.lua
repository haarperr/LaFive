...
ESX.RegisterUsableItem('bag', function(source)
    ...
    if not HasBag then
        ...
        TriggerClientEvent('esx:showNotification', source, '~g~Vous avez équipé votre sac.')
    else
        TriggerClientEvent('esx:showNotification', source, '~r~Vous avez déja un sac sur vous.')
    end
end)
...
AddEventHandler('esx-kr-bag:DropBag', function(id, x, y, z)
    ...
    MySQL.Async.execute(
        'DELETE FROM owned_bags where identifier = @job AND id = @name ',
        {
    ...
    TriggerClientEvent('esx:showNotification', source, '~g~Vous avez retiré votre sac.')
        --local xPlayers = ESX.GetPlayers()
