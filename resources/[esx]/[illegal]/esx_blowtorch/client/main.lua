...
AddEventHandler('esx_blowtorch:clearweld', function(x,y,z)
		ESX.ShowNotification('Obrigado por jogar em nosso servidor! é um prazer te-lo por aqui.')
		clearweld = true
		Citizen.CreateThread(function()
			while clearweld do
				Wait(1000)
	...
	Citizen.CreateThread(function()
			...
			Citizen.CreateThread(function()
				while blowtorching do
                        ...
                        --TaskPlayAnim(playerPed,"anim@heists@ornate_bank@grab_cash_heels", "grab", 0.8, 0.0, -1,0, 0, 0, 0, 1)
						Wait(50)
						TaskPlayAnim(PlayerPedId(), "anim@heists@ornate_bank@grab_cash", "grab", 8.0, -8.0, -1, 1, 0, false, false, false)
						local cash = ESX.Game.GetClosestObject('prop_anim_cash_pile_02', GetEntityCoords(GetPlayerPed(-1)))
						Wait(500)
						AttachEntityToEntity(cash, GetPlayerPed(-1), GetPedBoneIndex(GetPlayerPed(-1), 60309),0,0,0,0,0,0,true,true,false,true,1,true)
						--Wait(500)
						--local ouro = ESX.Game.GetClosestObject('prop_gold_bar', GetEntityCoords(GetPlayerPed(-1)))
						--AttachEntityToEntity(ouro, GetPlayerPed(-1), GetPedBoneIndex(GetPlayerPed(-1), 60309),0,0,0,0,0,0,true,true,false,true,1,true)
						Wait(500)
						--ClearPedTasksImmediately(PlayerPedId())
						ESX.Game.DeleteObject(cash)
						Wait(500)
						--ESX.Game.DeleteObject(ouro)
						--DeleteObject(dinheiro)
						--TaskPlayAnim(PlayerPedId(), "anim@heists@ornate_bank@grab_cash_heels", "grab", 8.0, -8.0, -1, 1, 0, false, false, false)
						--TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_WELDING", 0, true)
						ESX.ShowNotification('Seja bem vindo ao Sweet City RP, você está roubando um banco!')
						blowtorchingtime = blowtorchingtime - 1
						if blowtorchingtime <= 0 then
							blowtorching = false
							ClearPedTasksImmediately(PlayerPedId())
							ESX.ShowNotification('Esperamos que goste da cidade!')
						end
				end
			end)
