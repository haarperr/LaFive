			...
			if itemQuantity <= 0 then
				...
				if (rand >= 98) then
					SetTimeout(1500, function()
						...
						xPlayer.addInventoryItem('essence1', 1)
						TriggerClientEvent('esx:showNotification', source, _U('not_enough_raffine'))
...
local function Sell(source, zone)
	...
	if PlayersSelling[source] == true then
		...
		if zone == 'SellFarm' then
			if xPlayer.getInventoryItem('essence1').count <= 0 then
				essence = 0
			else
				essence = 1
			end
			...
			if essence == 0 then
				...
				return
			elseif xPlayer.getInventoryItem('essence1').count <= 0 then
				...
				if (essence == 1) then
					SetTimeout(1500, function()
						local money = math.random(80,90)
						local argent = math.random(80,90)
						xPlayer.removeInventoryItem('essence1', 1)
