...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('cidre_clothes_civil'), value = 'citizen_wear'},
				{label = _U('cidre_clothes_cidre'), value = 'cidre_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'cidre_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'cidre_actions_menu'
...
function OpenCidreActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cidre_actions',
		{
			title    = 'Cidre',
			align    = 'top-left',
			elements = elements
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'cidre', function(data, menu)
					menu.close()
				end)
			end
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'cidre', vehicleProps)
...
function OpenMobileCidreActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_cidre_actions',
		{
			title    = 'cidre',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill1', GetPlayerServerId(closestPlayer), 'society_cidre', 'Cidrier', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_cidrier:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_cidrier:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_cidrier:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_cidrier:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_cidrier:hasEnteredMarker', function(zone)
	if zone == 'PommeFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'cidre'  then
		CurrentAction     = 'pomme_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementCidre' and PlayerData.job ~= nil and PlayerData.job.name == 'cidre'  then
		CurrentAction     = 'cidre_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == 'cidre'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'cidre'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'CidrierActions' and PlayerData.job ~= nil and PlayerData.job.name == 'cidre' then
		CurrentAction     = 'cidre_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'cidre' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'cidre' then
...
AddEventHandler('esx_cidrier:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'PommeFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'cidre' then
		TriggerServerEvent('esx_cidrier:stopHarvest')
	end  
	if (zone == 'TraitementCidre' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'cidre' then
		TriggerServerEvent('esx_cidrier:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'cidre' then
		TriggerServerEvent('esx_cidrier:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.CidrierActions.Pos.x, Config.Zones.CidrierActions.Pos.y, Config.Zones.CidrierActions.Pos.z)
		...
		AddTextComponentString("Cidrier")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'cidre' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'cidre' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_cidrier:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_cidrier:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'cidre' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'pomme_harvest' then
					TriggerServerEvent('esx_cidrier:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_cidrier:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'cidre_traitement' then
					TriggerServerEvent('esx_cidrier:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_cidrier:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'cidre_actions_menu' then
					OpenCidreActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'cidre', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'cidre' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileCidreActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)