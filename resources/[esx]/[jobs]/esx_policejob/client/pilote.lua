...
function PiloteMenuTransport(menu)
    ...
    if distancePNJPaleto > 100 then
        transportPaleto = NativeUI.CreateItem("Direction Poste paleto bea", "~b~paleto bea")
    ...
    if distancePNJMissionRow > 100 then
        transportMissionRaw = NativeUI.CreateItem("Direction Poste Mission Raw", "~b~Mission Raw")
    ...
    menu.OnItemSelect = function(sender, item, index)
        if item == transportPaleto then
            ...
            if distanceCloseVeh <= 3.0 then
                ...
                VehBlock = ESX.Game.GetVehiclesInArea(ZoneDeSpawnMissionRaw.coords, 10)
                for _, voiture in pairs(VehBlock) do
                    ESX.Game.DeleteVehicle(voiture)
                end

                ESX.Game.SpawnVehicle("Buzzard2", ZoneDeSpawnMissionRaw.coords, 95.0, function(vehicule)
        ...
        elseif item == transportMissionRaw then
            ...
            if distanceCloseVeh <= 3.0 then
                ...
                local VehBlock = ESX.Game.GetVehiclesInArea(ZoneDeSpawnPaleto.coords, 10)
                for _, voiture in pairs(VehBlock) do
                    ESX.Game.DeleteVehicle(voiture)
                end

                ESX.Game.SpawnVehicle("Buzzard2", ZoneDeSpawnPaleto.coords, 95.0, function(vehicule)
                    SetVehicleMaxMods(vehicule)
                    ped = CreatePedInsideVehicle(vehicule, 6, GetHashKey("s_m_m_pilot_02"), -1, true, false)
                    --TaskVehicleEscort(ped, vehicule, veh, -1, 320.0, 1074528293, 1.0, 1, 30.0)
                    SetCanAttackFriendly(ped, false, true)
