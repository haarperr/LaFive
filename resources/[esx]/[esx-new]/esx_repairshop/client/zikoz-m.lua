...
function AddRepairMenu(menu)
    ...
    local lockpickmenu = _menuPool:AddSubMenu(menu, "Lockpick", nil, nil, "shopui_title_supermod", "shopui_title_supermod")

    local ventemenu = _menuPool:AddSubMenu(menu, "Vendre des pièces autos", nil, nil, "shopui_title_supermod", "shopui_title_supermod")
    ...
    local lock = NativeUI.CreateItem("Achetez", "")
    lockpickmenu.SubMenu:AddItem(lock)
    lock:RightLabel("3500$")

    local venteitem = NativeUI.CreateItem("Vendre", "")
    ventemenu.SubMenu:AddItem(venteitem)
    venteitem:RightLabel("~g~20000$")
    ...
    lockpickmenu.SubMenu.OnItemSelect = function(menu, item)
    if item == lock then
        TriggerServerEvent("BuyLock")
        ESX.ShowAdvancedNotification("Mécano", "Préparation...", "", "CHAR_CARSITE3", 1)
        Citizen.Wait(0)
        ESX.ShowNotification("3")
        Citizen.Wait(3)
        ...
        ESX.ShowNotification("1")
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Mécano", "~g~Lockpick", "", "CHAR_CARSITE3", 1)
        _menuPool:CloseAllMenus(true)
        end
    end

    ventemenu.SubMenu.OnItemSelect = function(menu, item)
    if item == venteitem then
        _menuPool:CloseAllMenus(true)
        notify('~w~Vous êtes entrain de vendre un ~b~Moteur ...')
        FreezeEntityPosition(GetPlayerPed(-1),true)
        Wait(5000)
        TriggerServerEvent('vendreMotor')
...
local blips = {
    {colour=17, id=402, x = 538.44665527344, y = -176.42123413086, z = 54.487968444824},
    {colour=17, id=402, x = 1188.0888671875, y = 2641.0961914063, z = 38.441184997559},
    {colour=17, id=402, x = 1954.830078125, y = 3838.2878417969, z = 32.177833557129},
    {colour=17, id=402, x = 1666.5975341797, y = 4969.1127929688, z = 42.255146026611},
    {colour=17, id=402, x = 102.11338806152, y = 6616.9047851563, z = 32.474632263184},
    {colour=17, id=402, x = 223.67, y = 2599.84, z = 45.57},
...
Citizen.CreateThread(function()
    for _, info in pairs(blips) do
        ...
        AddTextComponentString('Station de réparation')
