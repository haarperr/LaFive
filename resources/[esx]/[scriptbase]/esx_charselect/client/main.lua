    ...
    Citizen.CreateThread(function()
        ...
        if status == true then
            while enable == true do
                Citizen.Wait(1)
                ESX.ShowHelpNotification("Appuyer sur ~INPUT_FRONTEND_ACCEPT~ pour accéder à LaFive.")
        ...
        if status == false then
            while enable == true do
                Citizen.Wait(1)
                ESX.ShowHelpNotification("Appuyer sur ~INPUT_FRONTEND_ACCEPT~ pour passer a la creation de votre personnage.")
                RequestAnimDict("mp_character_creation@customise@male_a")
                TaskPlayAnim(PlayerPedId(), "mp_character_creation@customise@male_a", "loop", 8.0, -8.0, -1, 0, 1, 0, 0, 0)
                if IsControlJustReleased(1, 201) then
                    CreateChar()
